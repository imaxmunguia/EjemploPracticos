...
Namespace Telerik.Examples.WinControls.GridView.Spreadsheet
	Partial Class Form1
		...
		Private Sub StyleSelectedCell(ByVal cell As GridHeaderCellElement)
			...
			For Each selectedCell As GridViewCellInfo In Me.gridView.SelectedCells
				If cell.ColumnInfo.Index = selectedCell.ColumnInfo.Index Then
					If cell.ColumnInfo.Equals(Me.gridView.CurrentColumn) Then
						current = True
					Else
						selected = True
					End If
		...
		Private Sub SetStyle(ByVal style As GridViewCellStyle)
			If style.Font IsNot Nothing Then
		...
		Private Property BaseFont() As Font
			Get
				...
				Try
					Me.font = New Font(New FontFamily("Calibri"), 8.0F, FontStyle.Bold)
		...
		Private Function GetColumnName(ByVal columnIndex As Integer) As String
			...
			Do While cycle > 0
				Dim letterIndex As Integer = cycle Mod alphabetLength - 1

				result.Insert(0, ChrW(letterIndex + AscW("A"c)))
