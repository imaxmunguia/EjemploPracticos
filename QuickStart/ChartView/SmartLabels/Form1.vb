...
Namespace Telerik.Examples.WinControls.ChartView.SmartLabels
	Public Partial Class Form1
		...
		Public Sub New()
            ...
            AddHandler Me.radCheckBox3.ToggleStateChanged, AddressOf radCheckBox3_ToggleStateChanged
            AddHandler Me.radCheckBox4.ToggleStateChanged, AddressOf radCheckBox4_ToggleStateChanged
		...
		Private Sub InitializeScatter()
			...
			DirectCast(scatter.VerticalAxis, LinearAxis).Maximum = 110
			DirectCast(scatter.VerticalAxis, LinearAxis).DesiredTickCount = 12
			DirectCast(scatter.VerticalAxis, LinearAxis).Title = "Price(USD/per sq.ft.)"
			DirectCast(scatter.HorizontalAxis, LinearAxis).Maximum = 110
			DirectCast(scatter.HorizontalAxis, LinearAxis).DesiredTickCount = 12
			DirectCast(scatter.HorizontalAxis, LinearAxis).Title = "Property area (sq.ft.)"

			DirectCast(smartScatter.VerticalAxis, LinearAxis).Maximum = 110
			DirectCast(smartScatter.VerticalAxis, LinearAxis).DesiredTickCount = 12
			DirectCast(smartScatter.VerticalAxis, LinearAxis).Title = "Price(USD/per sq.ft.)"
			DirectCast(smartScatter.HorizontalAxis, LinearAxis).Maximum = 110
			DirectCast(smartScatter.HorizontalAxis, LinearAxis).DesiredTickCount = 12
			DirectCast(smartScatter.HorizontalAxis, LinearAxis).Title = "Property area (sq.ft.)"
		End Sub
        ...
        Private Sub radChartView_LabelFormatting(sender As Object, e As ChartViewLabelFormattingEventArgs)
            If Me.radCheckBox2.Checked Then
                If Me.radCheckBox4.Checked Then
                    e.LabelElement.BorderColor = DirectCast(e.LabelElement.Parent, DataPointElement).BackColor
                Else
		...
		Private Sub radSpinEditor1_ValueChanged(sender As Object, e As EventArgs)
			For Each controller As ChartViewController In Me.radChartView2.Controllers
				...
				If control IsNot Nothing Then
					control.Strategy.DistanceToLabel = CInt(Me.radSpinEditor1.Value)
					Me.radChartView2.View.PerformRefresh(Me.radChartView2.View, False)
				End If
			Next
		End Sub

        Private Sub radCheckBox4_ToggleStateChanged(sender As Object, args As StateChangedEventArgs)
            Me.radChartView1.View.PerformRefresh(Me.radChartView1.View, False)
            Me.radChartView2.View.PerformRefresh(Me.radChartView1.View, False)
        End Sub
    End Class
