...
Namespace Telerik.Examples.WinControls.ComboBoxAndListBox.ComboBox.Binding
	Partial Public Class Form1
		...
		Private Sub BindToArray()
			...
			result(0) = New DataItem("Belgium", My.Resources.BE)
			result(1) = New DataItem("Germany", My.Resources.DE)
			result(2) = New DataItem("India", My.Resources._IN)
			result(3) = New DataItem("France", My.Resources.FR)
			result(4) = New DataItem("USA", My.Resources.US)
			result(5) = New DataItem("Russia", My.Resources.RU)
			result(6) = New DataItem("Japan", My.Resources.JP)
			result(7) = New DataItem("China", My.Resources.CN)
			result(8) = New DataItem("Bulgaria", My.Resources.BUL)
			result(9) = New DataItem("Switzerland", My.Resources.CH)
		...
		Private Function FillFilesTable(ByVal path As String) As DataTable
			...
			dt.Columns.Add("LastAccessTime", GetType(DateTime))
		...
		Private Function GetCustomerList() As BindingList(Of Customer)
			Dim customerList As BindingList(Of Customer) = New BindingList(Of Customer)()
		...
		Private Sub CustomerBindingSource_AddingNew(ByVal sender As Object, ByVal e As AddingNewEventArgs) Handles CustomerBindingSource.AddingNew
			Dim newCustomer As New Customer()
		...
		Private Sub OnRadioBinding_ToggleStateChanged(ByVal sender As Object, ByVal args As StateChangedEventArgs) Handles radRadioDB.ToggleStateChanged, radRadioArray.ToggleStateChanged, radRadioDataTable.ToggleStateChanged, radRadioUnbound.ToggleStateChanged
